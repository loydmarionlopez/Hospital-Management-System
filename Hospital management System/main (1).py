...
db = Database("PatientManagement.db")
root = Tk()
...
title = Label(entries_frame, text="Patient Management System", font=("Calibri", 18, "bold"), bg="#2b2d4a", fg="white")
...
lbldoctor = Label(entries_frame, text="Doctor", font=("Calibri", 16), bg="#2b2d4a", fg="white")
lbldoctor.grid(row=2, column=2, padx=10, pady=10, sticky="w")
txtdoctor = Entry(entries_frame, textvariable=doctor, font=("Calibri", 16), width=30)
txtdoctor.grid(row=2, column=3, padx=10, pady=10, sticky="w")
...
def add_patient():
    if txtName.get() == "" or txtAge.get() == "" or txtdob.get() == "" or txtdoctor.get() == "" or comboGender.get() == "" or txtContact.get() == "" or txtAddress.get(
            1.0, END) == "":
        messagebox.showerror("Erorr in Input", "Please Fill All the Details")
        return
    db.insert(txtName.get(),txtAge.get(), txtdob.get() , txtdoctor.get() ,comboGender.get(), txtContact.get(), txtAddress.get(
            1.0, END))
...
def update_patient():
    if txtName.get() == "" or txtAge.get() == "" or txtdob.get() == "" or txtdoctor.get() == "" or comboGender.get() == "" or txtContact.get() == "" or txtAddress.get(
        ...
        return
    db.update(row[0], txtName.get(), txtAge.get(), txtdob.get(), txtdoctor.get(), comboGender.get(), txtContact.get(),
              txtAddress.get(
                  1.0, END))
...
def SearchRecord():
    ...
    if SEARCH.get() != "":
        ...
        tv.delete(*tv.get_children())
        #open database
        conn = sqlite3.connect('PatientManagement.db')
        #select query with where clause
        cursor=conn.execute("SELECT * FROM patient WHERE name LIKE ?", ('%' + str(SEARCH.get()) + '%',))
...
btnAdd = Button(btn_frame, command=add_patient, text="Add Details", width=15,font=("Calibri", 16, "bold"), fg="steelblue",
                bg="WHITE", bd=0).grid(row=0, column=0)
btnEdit = Button(btn_frame, command=update_patient, text="Update Details", width=15, font=("Calibri", 16, "bold"),
                 fg="steelblue", bg="WHITE",
                 bd=0).grid(row=0, column=1, padx=10)
btnDelete = Button(btn_frame, command=delete_patient, text="Delete Details", width=15, font=("Calibri", 16, "bold"),
                   fg="steelblue", bg="WHITE",
